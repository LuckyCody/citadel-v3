...
**Test-parallelism observation** (found while executing phases, reproduced twice): two tests can fail under default parallel `cargo test` load and pass deterministically with `--test-threads=1` — which is exactly how ci.yml runs the workspace suite, so upstream CI never sees them: `citadel-api`'s `exploit_scoped_admin_*_partial_overlap_listing` (reproduced twice) and `citadel-keystore`'s `vertical_slice.rs` P063 tests (env-var race, reproduced on the unmodified base). Not product bugs; shared-state test-hygiene items worth one look.
