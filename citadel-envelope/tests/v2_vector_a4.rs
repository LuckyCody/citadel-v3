...
use citadel_envelope::{
    v2_test_vectors::{deterministic_envelope_a4, open_a4},
    HybridP384MlKem1024Provider, KemProvider,
};
use hkdf::Hkdf;
...
struct CheckedVector {
    name: String,
    recipient_p384_scalar_hex: String,
    mlkem_d_hex: String,
    mlkem_z_hex: String,
    ephemeral_p384_scalar_hex: String,
    mlkem_m_hex: String,
    nonce_hex: String,
...
#[test]
fn deterministic_a4_vector_matches_independent_transcript_reconstruction() {
    let vector: CheckedVector =
        serde_json::from_str(include_str!("vectors/envelope_v2_a4.json")).unwrap();
    assert_eq!(vector.name, "envelope-v2-a4-deterministic-001");
    ...
    let (pk, sk, shared_secret, kem_ct, envelope) = deterministic_envelope_a4(
        array(&vector.recipient_p384_scalar_hex),
        array(&vector.mlkem_d_hex),
        array(&vector.mlkem_z_hex),
        array(&vector.ephemeral_p384_scalar_hex),
    ...
    assert_eq!(shared_secret.len(), 80);
    assert_eq!(kem_ct.len(), 1665);

    let mut header = [0u8; 98];
    header[..4].copy_from_slice(b"CTD2");
    header[4..10].copy_from_slice(&[2, 0, 0xA4, 0xC1, 0xB1, 0]);
    header[10..12].copy_from_slice(&be16(98));
    header[12..14].copy_from_slice(&be16(1665));
    header[14..22].copy_from_slice(&(PLAINTEXT.len() as u64).to_be_bytes());
    header[22..54].copy_from_slice(&Sha3_256::digest(
        HybridP384MlKem1024Provider::public_key_bytes(&pk),
    ));
    ...
    let opened = open_a4(&sk, &envelope, AAD, CONTEXT).unwrap();
