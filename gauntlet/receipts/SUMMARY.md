...
| **4** Constant-time | dudect (+DATA/MicroWalk follow-on) | prior evidence in `../../docs/security/TIMING.md` | attacker-controlled-input classes pass; key-material effect documented, binary CT analysis pending |
...
| **8** CT proof | ctgrind (valgrind memcheck) on ML-KEM decap | ✅ **EXECUTED — resolved** | Citadel's own code: **0 secret-dependent branches**. Residual localizes to ml-kem 0.3.2 `sample_poly_cbd` — a real secret-indexed lookup `ONES[val.0]`, but for ML-KEM-768 (η=2) the table is 32B/one cache line + index always in-range ⇒ **not practically exploitable** (matches docs/security/TIMING.md). Dependency property, not Citadel code. `receipts/tier8_ct.txt`. |
...
1. ~~Clear Tier 2b optics~~ **DONE 2026-07-20** — bumped libcrux dev-oracle to 0.0.10
   ...
   ignore) and documented in `docs/security/SUPPLY_CHAIN.md`. Suite unchanged at 353/0/8.
